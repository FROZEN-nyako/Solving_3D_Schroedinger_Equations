...
def initialize(params):
    ...
    T_diagonals = np.array([44 / 13 / a ** 2 / (2 * M)])
    Ts = np.repeat([-3 / 13 / a ** 2 / (2 * M)], 12)
    Te = np.repeat([-3 / 26 / a ** 2 / (2 * M)], 24)
    Tc = np.repeat([-3 / 39 / a ** 2 / (2 * M)], 16)

    elements = np.concatenate((T_diagonals, Ts, Te, Tc))
